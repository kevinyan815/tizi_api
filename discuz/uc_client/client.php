...
//梯子网连接论坛专用 author:zhangxiaomign@tizi.com
function uc_user_insert($uid,$username, $password, $email, $questionid = '', $answer = '', $regip = '') {
	return call_user_func(UC_API_FUNC, 'user', 'insert', array('uid'=>$uid,'username'=>$username, 'password'=>$password, 'email'=>$email, 'questionid'=>$questionid, 'answer'=>$answer, 'regip' => $regip));
}

//梯子网连接论坛专用 author:zhangxiaomign@tizi.com
function uc_select_count($sql) {
	return call_user_func(UC_API_FUNC, 'user', 'select_sql',array('sql'=>$sql));
}

//梯子网连接论坛专用 author:zhangxiaomign@tizi.com
function uc_getuserbyuid($uid) {
	return call_user_func(UC_API_FUNC, 'user', 'getuserbyuid',array('uid'=>$uid));
}

//梯子网连接论坛专用 author:zhangxiaomign@tizi.com
function uc_updatepwdbyuid($uid,$new_pwd) {
	return call_user_func(UC_API_FUNC, 'user', 'updatepwdbyuid',array('uid'=>$uid,'password'=>$new_pwd));
}

//梯子网连接论坛专用 author:zhangxiaomign@tizi.com
function uc_getgroupidbyuid($uid) {
	return call_user_func(UC_API_FUNC, 'user', 'getgroupbyuid',array('uid'=>$uid));
}
