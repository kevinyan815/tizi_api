...
class usermodel {
	...
	function get_groupid_by_uid($uid){
		$arr = $this->db->fetch_first("SELECT groupid FROM ".DISCUZ_DBTABLEPRE."common_member WHERE uid='$uid'");
		return $arr['groupid'];
	}
	function update_pwd_by_uid($uid,$pwd) {
		if(!$this->db->query("UPDATE ".UC_DBTABLEPRE."members SET password='$pwd' WHERE uid='$uid'")){
			//exit('ddd');
		}
		if(!$this->db->query("UPDATE ".DISCUZ_DBTABLEPRE."common_member SET password='$pwd' WHERE uid='$uid'")){
			//exit('eee');
		}
	...
	function add_user($username, $password, $email, $uid = 0, $questionid = '', $answer = '', $regip = '') {
		...
		//同时添加到discuz中 开始 必须现在config.inc.php中定义常量 "DISCUZ_DBTABLEPRE"，就是discuz的库及表前缀
		$this->db->query("INSERT INTO ".DISCUZ_DBTABLEPRE."common_member SET uid='$uid', username='$username', password='$password', email='$email', adminid='0', groupid='10', regdate='".$this->base->time."', credits='0', timeoffset=''");
