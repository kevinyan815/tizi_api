...
class usercontrol extends base {
	...
	function onselect_sql(){
		$this->init_input();
		$sql = $this->input('sql');
		$data = $_ENV['user']->select_sql($sql);
		return $data;
	}
	...
	function ongetuserbyuid(){
		$this->init_input();
		$uid = $this->input('uid');
		$arr = $_ENV['user']->get_user_by_uid($uid);
		return $arr;
	}
	function ongetgroupbyuid(){
		$this->init_input();
		$uid = $this->input('uid');
		$groupid = $_ENV['user']->get_groupid_by_uid($uid);
		return $groupid;
	}
	function onupdatepwdbyuid(){
		$this->init_input();
		$uid = $this->input('uid');
		$password = $this->input('password');
		//echo $uid.'---'.$password;exit;
		$arr = $_ENV['user']->update_pwd_by_uid($uid,$password);
		return $arr;
	}
