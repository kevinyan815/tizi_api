...
class DB_Sql {
    ...
    function ping(){
        $LinkName=$this->LinkName;
        global $$LinkName;
        $res_ping = @mysql_db_query($this->Database,"select 'x'",$$LinkName);
        return $res_ping;
    }
    ...
    function parse_query_string($Query_String) {
        ...
        if (($this->PageSize>0) && ($this->AbsolutePage>0) && eregi("select ",$Query_String)) {
            ...
            $strsql.= '/* '.$_SERVER['HTTP_HOST'].' in '.$_SERVER['PHP_SELF'].' */';
    ...
    function query($Query_String) {
        ...
        $Query_String.= '/* '.$_SERVER['HTTP_HOST'].' in '.$_SERVER['PHP_SELF'].' */';
    ...
    function get_results($query , $oMethod = 'A'){
        ...
        if($oMethod=='A'){ //返回二维数组
            while($t = @mysql_fetch_assoc($this->Query_ID)){
                $Results[]=$t;
                $this->Row += 1;
            }
