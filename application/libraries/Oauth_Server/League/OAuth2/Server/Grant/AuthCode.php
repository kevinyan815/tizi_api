...
class AuthCode implements GrantTypeInterface {
    ...
    public function newAuthoriseRequest($type, $typeId, $authParams = array())
    {
        ...
        $authCode = SecureKey::encrypt(substr(sha1(rand()), 0, 6).'.'.$authParams['user_id']);

        // Remove any old sessions the user might have
        //$this->authServer->getStorage('session')->deleteSession($authParams['client_id'], $type, $typeId);
        $sessionDetails = $this->authServer->getStorage('session')->getSession($authParams['client_id'], $type, $typeId);
		if (!empty($sessionDetails)){
		
			$sessionId = $sessionDetails['id'];
			$this->authServer->getStorage('session')->removeAuthCode($sessionId);
		...
		if (isset($authParams['scopes']) && !empty($authParams['scopes'])) {
			
			$authParams['scopes'] = $this->authServer->getStorage('scope')->getScopes(explode(',', $authParams['scopes']));
    ...
    public function completeFlow($inputParams = null)
    {
		...
		if($authCodeDetails['user_id'] != $user_id){

            throw new Exception\ClientException('code error');
		}

		$accessTokenDetails = $this->authServer->getStorage('session')->getUserAccessToken($authParams['client_id'], $user_id);
		if(!empty($accessTokenDetails) && $accessTokenDetails['access_token_expires'] > time()){

			$accessToken =  $accessTokenDetails['access_token'];

		}else{
			
			if (isset($accessTokenDetails['id']) && !empty($accessTokenDetails['id']))
				$this->authServer->getStorage('session')->removeAccessToken($accessTokenDetails['id']);
